        ...
        <tbody>
            @php
                ...
                for($i=0; $i<$counting; $i++){
                    ...
                    $site_count = count($detail[$i]['purchase']);
                    $string = '';
                    $qty = 0;
                    if($site_count>0){
                        for($j=0; $j<$site_count; $j++){
                            if($detail[$i]['purchase'][$j]['purchase_request_id'] == $purchase_id){
                                $qty++;
                                $string .= '<tr>';
                                    $string .= '<td style="text-align: right; border: 1px solid #dddddd;">'.$detail[$i]['no'].'.'.$qty.'</td>';
                                    $string .= '<td style="border: 1px solid #dddddd; text-indent: 30px;" colspan="4">'.$detail[$i]['purchase'][$j]['site_id'].'-'.$detail[$i]['purchase'][$j]['site_name'].' '.$detail[$i]['purchase'][$j]['site_area'].'</td>';
                                $string .= '</tr>';
                            }
                        }
                    }
                    $total = 0;
                    $total = $qty * $detail[$i]['cost'];
                    ...
                    echo '<tr>';
                        ...
                        echo '<td style="text-align: center; border: 1px solid #dddddd;">'.$qty.' '.$detail[$i]['unit'].'</td>';
